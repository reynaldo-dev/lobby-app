...
import { useGetGivenRecognitionsQuery, useGetReceivedRecognitionsQuery, useMarkRecognitionAsReadMutation } from "../../../redux/services/recognitions/recognitions.service";
...
const RecognitionList = ({ type }: RecognitionListProps) => {
    ...
    const navigation = useNavigation<NavigationProp<RootStackParamList, "Recognitions">>();

    const { data: receivedRecognitions, isLoading: isReceivedLoading, refetch: refetchReceivedRecognitions } = useGetReceivedRecognitionsQuery(user?.id || '');
    const { data: givenRecognitions, isLoading: isGivenLoading } = useGetGivenRecognitionsQuery(user?.id || '');
    ...
    const renderItem = ({ item: recognition }: { item: IRecognition }) => {
        ...
        return (
            <Box key={recognition.id} width={"95%"} alignSelf={"center"}>
                <Pressable onPress={() => handlePress(recognition)} style={{ backgroundColor: backgroundColor }}>
                    {({ isPressed }) => (
                        <VStack
                            px={2}
                            py="3"
