...
import { useGetRedeemablesQuery } from "../../../redux/services/reedemables/reedemeables.service";
import Layout from "../../../shared/layout/Layout";

export const Redeemables = () => {
    const navigation = useNavigation<NavigationProp<RootStackParamList, "Redeemables">>();
    const { data: redeemables, isLoading, isError, error } = useGetRedeemablesQuery();

    const [selectedTokenType, setSelectedTokenType] = React.useState<string | undefined>(undefined);


    const tokenTypes = React.useMemo(() => {
        const types = redeemables ? [...new Set(redeemables.map(r => r.token.name))] : [];
        return ['Todos', ...types];
    }, [redeemables]);

    const filteredRedeemables = redeemables?.filter(redeemable => !selectedTokenType || selectedTokenType === 'Todos' || redeemable.token.name === selectedTokenType);
